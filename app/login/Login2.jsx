...
const page = () => {
  ...
  async function Submit(email, password) {
    ...
    if (!parsed.success) {
      ...
      setError(
        `${fieldErrors.email || ""} ${fieldErrors.password || ""}`.trim()
      );
      setLoading(false);
      return;
    }
                ...
                >
                  ...
                  <div className="flex items-center text-sm text-muted-foreground gap-1.5">
                    <button
                      type="button"
                      className={`${
                        pressed ? "bg-gray-500 scale-95" : ""
                      } inline-flex items-center gap-2 rounded-md border px-2 py-2 hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring transition-all`}
